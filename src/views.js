...
const updateCounterUI = (board) => {
    ...
    counterEl.innerHTML = `Movimientos: ${board.getCounter()}`;
};
...
const checkIfFinished = (board, timer) => {
    ...
    if (board.getStatus() === 'finished') {
        ...
        messageEl.innerHTML = `Te tardaste ${m} minutos ${s} segundos utilizando solo: ${moves} movimientos pa`;
        ...
        for (let i = 0; i < scores.length; i++) {
            ...
            pEl.innerHTML = `${i + 1}. Movimientos: ${scores[i].moves} Tiempo: ${scores[i].time} hace: ${moment(scores[i].timeStamp).fromNow()}`;
